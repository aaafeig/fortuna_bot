...
def slots_menu(user_id):
    ...
    btn_rules = types.InlineKeyboardButton("📚 Правила игры", callback_data="slots_rules")
    btn_bck = types.InlineKeyboardButton("⬅️ Назад", callback_data="main_menu")
    markup.row(btn_play, btn_rules)
    markup.row(btn_bck)
    bot.send_message(user_id, "🎰 Добро пожаловать в игру 'Слоты'! Выберите действие:", reply_markup=markup)
