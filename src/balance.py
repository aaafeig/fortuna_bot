...
def ensure_user_data(user_id, username=None):
    ...
    if "balance" not in user_balances[user_id] or not isinstance(
        user_balances[user_id]["balance"], (int, float)
    ):
        user_balances[user_id]["balance"] = DEFAULT_BALANCE
    if "username" not in user_balances[user_id] or not isinstance(
        user_balances[user_id]["username"], str
    ):
