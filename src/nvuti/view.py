...
def nvuti_menu(user_id):
    ...
    btn_rules = types.InlineKeyboardButton("📚 Правила игры", callback_data="nvuti_rules")
    btn_bck = types.InlineKeyboardButton("⬅️ Назад", callback_data="main_menu")
    markup.row(btn_play, btn_rules)
    markup.row(btn_bck)
    bot.send_message(user_id, "🎲 Добро пожаловать в игру Nvuti! Выберите действие:", reply_markup=markup)
...
def nvuti_bet(message, chance):
    ...
    if bet <= 0:
        ...
        return
    if bet > balance:
        bot.send_message(user_id, f"❌ У вас недостаточно средств! Ваш баланс: 💵 {balance} монет.\nВведите ставку заново:")
        bot.register_next_step_handler(message, nvuti_bet, chance)
        return
    user_balances[user_id]["current_nvuti"] = {
        "chance": chance,
        "bet": bet
    }
    save_balances()
    ...
    markup = types.InlineKeyboardMarkup()
    btn_less = types.InlineKeyboardButton(f"Меньше (0 - {less_max})", callback_data="nvuti_less")
    btn_more = types.InlineKeyboardButton(f"Больше ({more_min} - 999999)", callback_data="nvuti_more")
    markup.row(btn_less, btn_more)
    bot.send_message(user_id, f"Вы выбрали шанс {chance}%.\n"
                              f"Ставка: {bet} монет.\n"
                              "Ваш выбор: меньше или больше?", reply_markup=markup)